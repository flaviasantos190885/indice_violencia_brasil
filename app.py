...
if st.session_state.pagina_selecionada == "📊 Dashboard de Análise":
    ...
    # --- ADICIONADO: Filtro dinâmico de cidade ---
    # Se um estado for selecionado, mostra as cidades correspondentes
    ...
    # Se não, o filtro de cidade fica desabilitado
    else:
        st.selectbox("Filtrar por Cidade (Opcional)", ["Selecione um estado primeiro"], disabled=True)
    ...
    if st.button("Calcular Estimativa"):
        ...
        # --- ADICIONADO: Aplica o filtro de cidade na previsão ---
        ...
        if len(df_filtrado_pred) < janela:
            ...
            with st.spinner("Calculando... O modelo está processando os dados."):
                ...
                # ... (resto da sua lógica de previsão continua aqui, sem alterações) ...
